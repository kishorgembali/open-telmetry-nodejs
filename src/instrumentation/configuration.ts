...
export function initInstrumentation(aditionalInstrumentations?: InstrumentationOption[], spanExporter?: SpanExporter) {
    if (process.env.OTEL_DEBUG_LOGGING?.toLowerCase() === "true") {
        diag.setLogger(new DiagConsoleLogger(), DiagLogLevel.DEBUG);
    }

    if (!process.env.npm_package_name) {
        throw new Error("No service package name found while creating resource for tracing");
    }

    if (!process.env.npm_package_version) {
        throw new Error("No service package version found while creating resource for tracing");
    }
